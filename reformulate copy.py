...
def identify_concept_hierarchy_prompt(cond: str, obj_samples: list):
    prompt = f"""# Task Description
You are an expert in data analysis, semantic web, and knowledge graphs. Given a "Query Term" and a list of "Sampled Table Values" from a database column, your task is to determine whether the Query Term is **sufficiently specific** compared to the context and granularity provided by the sampled table values.

# Evaluation Criteria & Definitions
1. **Specific**: The semantic scope of the Query Term is narrower than (a subtype/instance of) or equal to (at the same level of granularity as) the concepts represented by the sampled table values.
2. **Too Broad**: The semantic scope of the Query Term is significantly broader than the sampled table values. It acts as a hypernym (parent category) to the table values.
3. **Other**: Hard to determine

---
# Examples

## Example 1
Query Term: iPhone 14 Pro
Sampled Table Values: ["iPhone 13", "Samsung Galaxy S23", "Pixel 7", "Xiaomi 13"]
Output: Specific
...
# Actual Task
Query Term: {cond}
Sampled Table Values: {obj_samples}
Output: Specific | Too Broad | Other

Directly output the answer (Specific | Too Broad | Other), without any other text.
"""
    return prompt
...
def identify_semantic_gap(cond: str, obj_scores: dict):
    obj_pos, obj_neg = [], []
    for obj, score in obj_scores.items():
        if score > 1:
            ...
            obj_neg.append(obj)
    obj_samples = obj_pos[:5] + obj_neg[:5]
    # if format inconsistency
    format_prompt = identify_identifier_prompt(cond)
    # concept hierarchy prompt
    concept_hierarchy_prompt = identify_concept_hierarchy_prompt(cond, obj_samples)
    # synonym prompt
    synonym_prompt = identify_synonym_prompt(cond, obj_pos)

    responses = run_inference(
        [format_prompt, concept_hierarchy_prompt, synonym_prompt], temperature=0.7
    )
